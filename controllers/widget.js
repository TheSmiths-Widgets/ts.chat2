...
@ _fadeInSend
*/
function _fadeInSend(event){
    this.backgroundColor = _fadeInColor;
};

/*
@ _fadeOutSend
*/
function _fadeOutSend(event){
...
(function(){
    // Send Btn Icon
    $.faSendBtn.text = fa.icon('fa-paper-plane');
    $.faSendBtn.font = { fontFamily: fa.fontfamily, fontSize: 18 };

    // Send Btn Color
    $.sendBtn.backgroundColor = _fadeOutColor;
...
function customize(){
    ...
    if( args.sendBtn ){
        // Change Icon
        if( args.sendBtn.icon ) $.faSendBtn.text = fa.icon( args.sendBtn.icon );

        // Change Icon Color
        if( args.sendBtn.iconColor ) $.faSendBtn.color = args.sendBtn.iconColor;

        // Change Icon Size
        if( args.sendBtn.iconSize ) $.faSendBtn.font = { fontSize: args.sendBtn.iconSize, fontFamily: fa.fontfamily };

        // Change Color
        if( args.sendBtn.color ){
            $.sendBtn.backgroundColor = args.sendBtn.color;
            _fadeOutColor = args.sendBtn.color;
        }

        // Change FadeIn Color
        if( args.sendBtn.fadeIn ){
            _fadeInColor = args.sendBtn.fadeIn;
        }
